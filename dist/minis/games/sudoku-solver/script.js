...
const boardElement = isBrowser ? document.getElementById("board") : null;

const difficultySelect = isBrowser
  ? document.getElementById("difficulty")
  : null;

const newGameButton = isBrowser ? document.getElementById("newGame") : null;

const solveButton = isBrowser ? document.getElementById("solveBtn") : null;

const hintButton = isBrowser ? document.getElementById("hintBtn") : null;

const resetButton = isBrowser ? document.getElementById("resetBtn") : null;

const timerElement = isBrowser ? document.getElementById("timer") : null;

const messageElement = isBrowser ? document.getElementById("message") : null;

const headerStatus = isBrowser ? document.getElementById("headerStatus") : null;

const filledStat = isBrowser ? document.getElementById("filledStat") : null;

const hintStat = isBrowser ? document.getElementById("hintStat") : null;

const difficultyStat = isBrowser
  ? document.getElementById("difficultyStat")
  : null;

const difficultyDisplay = isBrowser
  ? document.getElementById("difficultyDisplay")
  : null;

const filledDisplay = isBrowser
  ? document.getElementById("filledDisplay")
  : null;

const puzzleState = isBrowser ? document.getElementById("puzzleState") : null;

const numberButtons = isBrowser
  ? document.querySelectorAll(".number-pad button")
  : [];
...
function generatePuzzle(difficulty) {
  ...
  const puzzleBoard = solved.map(row => [...row]);
...
function isValidMove(board, row, col, number) {
  for (let x = 0; x < 9; x++) {
    if (board[row][x] === number && x !== col) {
      return false;
    }
  }

  for (let x = 0; x < 9; x++) {
    if (board[x][col] === number && x !== row) {
      return false;
    }
  }
...
function enterNumber(number) {
  ...
  if (puzzle[row][col] !== 0) {
    updateStatus("That number is part of the puzzle.");
  ...
  if (!isValidMove(currentBoard, row, col, number)) {
    markError(row, col);

    updateStatus("That number conflicts with this row, column, or box.");
...
function giveHint() {
  if (!selectedCell) {
    ...
    for (let row = 0; row < 9; row++) {
      for (let col = 0; col < 9; col++) {
        if (puzzle[row][col] === 0 && currentBoard[row][col] === 0) {
          emptyCells.push({
            row,
            col,
          });
    ...
    if (emptyCells.length === 0) {
      updateStatus("No empty cells available.");
  ...
  if (puzzle[row][col] !== 0) {
    updateStatus("Select an empty cell for a hint.");
  ...
  for (let r = 0; r < 9; r++) {
    ...
    for (let c = 0; c < 9; c++) {
      ...
      if (!Number.isInteger(val) || val < 0 || val > 9) {
        throw new Error(
          `Invalid value ${val} at row ${r + 1}, column ${c + 1}.`
        );
      }
    }
  }
      ...
      for (let r = startRow; r < startRow + 3; r++) {
        for (let c = startCol; c < startCol + 3; c++) {
          ...
          if (val !== 0) {
            if (seen.has(val)) {
              throw new Error(
                `Subgrid starting at row ${startRow + 1}, column ${startCol + 1} has duplicate value ${val}.`
              );
            }
            seen.add(val);
          }
...
function resetPuzzle() {
  ...
  if (puzzleState) {
    puzzleState.textContent = "In Progress";
  }
}
...
function checkCompletion() {
  ...
  if (puzzleState) {
    puzzleState.textContent = "Completed";
  }

  if (headerStatus) {
    headerStatus.textContent = "Puzzle Complete";
  }

  updateStatus("🎉 Congratulations! Puzzle completed.");
...
function updateStats() {
  ...
  if (isBrowser) {
    const difficulty = capitalize(difficultySelect.value);
