  ...
  const documentMock = {
    ...
    querySelectorAll: selector => {
      if (
        ...
        selector === ".pad" ||
        selector === ".string" ||
        selector === ".string-hit-area" ||
        selector === "[data-string]" ||
        selector === "[data-fret]"
      ) {
        if (queriedElements.length === 0) {
          let cname = selector.startsWith(".")
    ...
    elementFromPoint: (x, y) =>
      queriedElements[0] || elements["string-layer"].children[0],
    addEventListener: (evt, cb) => {
...
function runInstrumentScript(instrument) {
  const { windowMock, documentMock, queriedElements, elements } =
    createMockDom();
  ...
  if (fs.existsSync(enginePath)) {
    ...
    if (instrument === "guzheng")
      sandbox.GuzhengEngine = sandbox.module.exports;
    if (instrument === "percussion")
      sandbox.PercussionEngine = sandbox.module.exports;
...
test("Instrument interaction tests", async t => {
  ...
  for (const instrument of instruments) {
    await t.test(
      `${instrument} responds to pointer and keyboard events`,
      async () => {
        const { documentMock, queriedElements, elements } =
          runInstrumentScript(instrument);
        ...
        assert.ok(
          targetElement,
          `Script for ${instrument} should have interactive elements`
        );
        ...
        let isActive =
          targetElement.className.includes("active") ||
        ...
        assert.ok(
          isActive,
          `Pointer down should set active state on ${instrument}. Got class: ${targetElement.className}`
        );
        ...
        assert.ok(
          isActiveKey,
          `Keydown should trigger active state on ${instrument}. Got class: ${targetElement.className}`
        );
  ...
  await t.test(
    "audio contexts respect explicit user gesture activation policy",
    () => {
      for (const instrument of instruments) {
        const { sandbox } = runInstrumentScript(instrument);
        // AudioContext should remain null or un-activated until interaction
        assert.equal(
          sandbox.audioContext === null || sandbox.audioContext === undefined,
          true,
          `Audio context in ${instrument} must not be eagerly instantiated before user interaction.`
        );
      }
    }
  );
